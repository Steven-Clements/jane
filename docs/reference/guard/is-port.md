...
Checks whether a value is a *valid TCP/UDP port number*. A port is defined as an integer between `0` and `65535` (inclusive). This helper never throws and never mutates input. It performs strict type checks and does not coerce strings or other values into numbers.
...
- This helper does not coerce strings (e.g., "80" is invalid).
