...
Checks whether a value is a *plain object*. A plain object is one created with `{}` or `new Object()`, where the prototype is exactly `Object.prototype`. This helper excludes `null`, arrays, class instances, and objects with custom prototypes. It never throws and never mutates input.
...
- This helper is stricter than `isObject`.
- Use `isObject` if you only need to exclude `null` and arrays.
