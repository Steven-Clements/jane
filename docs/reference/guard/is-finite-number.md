...
Checks whether a value is a finite primitive number. A finite number is a number primitive that is not NaN, Infinity, or -Infinity. This helper never throws and never mutates input. Use it when you need to confirm that a value is suitable for arithmetic, normalization, or any domain where numeric finiteness is required.
