...
Checks whether a value is a regular expression. This helper performs a strict `instanceof` check and never throws or mutates input. It accepts both RegExp literals and RegExp objects created with `new RegExp`, including wrapper objects created with `Object(...)`.
